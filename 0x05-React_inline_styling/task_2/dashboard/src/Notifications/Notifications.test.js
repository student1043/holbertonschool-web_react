...
describe('<Notifications />', () => {
  ...
  it('<Notifications /> renders <NotificationItem />', () => {
    const wrapper = shallow(<Notifications displayDrawer={true} />);
    expect(wrapper.find('NotificationItem').first().html()).toEqual("<li data-priority=\"default\" class=\"default_1tsdo2i\">No new notification for now</li>");
  });
...
describe('listNotifications has values in it', () => {
  ...
  it('the values', () => {
    ...
    expect(notificat.at(1).html()).toEqual(
      "<li data-priority=\"urgent\" class=\"urgent_137u7ef\">New resume available</li>"
    );
    expect(notificat.at(2).html()).toEqual(
      `<li data-priority="urgent" class=\"urgent_137u7ef\">${latestNotification}</li>`
    );
  });
});
...
describe('listNotifications does not have any values', () => {
  ...
  it('no values', () => {
    ...
    expect(notificat.html()).toEqual(
      "<li data-priority=\"default\" class=\"default_1tsdo2i\">No new notification for now</li>"
    );
  });
